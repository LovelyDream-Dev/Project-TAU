...
func draw_beat_ticks(BeatTime:float, tickHeight:float, tickWidth:float, tickColor:Color, rounded:bool):
	var xPosition = rootNode.get_timeline_position_x_from_song_position(BeatTime) + scrollContainer.playheadOffset + (CurrentMap.leadInTime * rootNode.pixelsPerSecond)
